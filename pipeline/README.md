...
| `npm run build:grammar` | noun gender, n-rule and adjective-agreement exercises, mined from the corpus and gated the same as everything else | `content/items/grammar.json`, `app/data/grammar.json` |
...
**`content/items/grammar.json`** — noun gender, the n-rule, and adjective
agreement, generated by `build-grammar.js`. Same "never authors Luxembourgish"
rule as everything above, applied to three different sources:

- `gender` items are read straight off `vocab.json` — no mining. Every noun
  there already carries LOD's own `gender`/`article`, unused by any screen
  until this deck; the exercise is just surfacing it.
- `nrule` items are mined from the corpus's own example sentences: for every
  adjacent word pair, `lib/nrule.js`'s checker is asked whether the rule is
  being followed *correctly* right there — the mirror image of what
  `validate.js`'s gate 2 flags as wrong. A pair with no finding either way is
  a clean, real instance, and both the kept-n and dropped-n spellings are
  independently confirmed against the lexicon before either becomes an option.
- `adjective` items ask which of an adjective's own *attested* forms LOD wrote
  in one specific sentence, using the same lexicon-form lookup `lib/cloze.js`
  already uses to find a word inside its own example. The distractors are
  other real forms of the same lemma, attested elsewhere in the corpus — wrong
  in that slot, never invented. An adjective attested in only one form across
  its own examples yields no item, the same "ship nothing rather than guess"
  rule `build-learn.js` follows for a cloze target it cannot locate.

Every candidate sentence is run through the same gate `build-vocab.js` and
`build-verbs.js` already run their own examples through (`lib/gate.js`)
before it can become an item — a sentence that mentions one word outside the
lexicon is dropped whole rather than shipping context the validator would
fail on.
