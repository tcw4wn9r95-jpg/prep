...
test('grammar: the shipped deck carries only the three declared kinds, each internally consistent', (t) => {
  ...
  for (const item of items) {
    ...
    seenIds.add(item.id);
    assert.ok(['gender', 'nrule', 'adjective'].includes(item.kind), `${item.id}: unknown kind "${item.kind}"`);
